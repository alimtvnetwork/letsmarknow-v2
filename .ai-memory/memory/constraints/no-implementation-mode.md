...
description: PERMANENT spec-only mode. Never write code, never enable Cloud/Supabase, never run migrations, never suggest tests/fixtures/seed data. Only spec files (02-spec/21-app/**) and memory.
type: constraint
---
...
- Fine-tune existing spec files in `02-spec/21-app/**` for clarity.
...
- User says "audit" → write findings to `02-spec/21-app/audit-{date}-{topic}.md`. Never propose code.
