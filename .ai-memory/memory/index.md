...
## Core
Spec lives in `02-spec/21-app/` — "By domain/feature" structure (00-overview/ … 20-roadmap/). Do not flatten or re-shape folders.
Conversation logging: append every user instruction verbatim to `02-spec/21-app/00-conversation-log.md` AND refactor the relevant structured spec file(s).
...
## Memories
- [Spec issue tracker](mem://features/spec-issue-tracker.md) — **1 SI open / 45 SIs closed.** Session 148 (2026-05-03): **SI-030 CLOSED** — 179 ULID `"01J..."` placeholders bulk-replaced with UUIDv7 stub `0190a4f1-6c5e-7c2a-9b3f-1234567890ab` across 29 files (audit-148). New `ulid-placeholder` lint sub-check (18th) registered in `22-infrastructure/09-ci-cd.md §2.1.1`. **SI-029** (privacy-pack legal copy) still blocked on counsel. Recent broad sweeps clean: `15-visualization/` (143), `17-admin-org/` (144), `01-information-architecture/` (145), `06-ui-ux/` (146 — 11 string drifts patched), `03-api-endpoints/` (147 — opened SI-030, now closed). **Next priorities:** broad sweeps on `12-history-undo/`, `16-notifications-updates/`, `22-infrastructure/`.
