...
## Memories
- [Spec issue tracker](mem://features/spec-issue-tracker.md) — **1 SI open + 8 audit findings open / 32 SIs closed.** Session 67 (2026-04-29): **closed S1 launch-blockers SP1 + SP2** from security-privacy audit (2 of 10 closed). `03-encryption.md §2` + §5 rewrote password hashing from bcrypt → Argon2id (m=64MB,t=3,p=4) per `09-auth-accounts/03-passwords-and-mfa.md §1` lock; added explicit "verifiers MUST reject bcrypt-prefixed hashes" rule. `05-share-link-security.md §6.1` aligned to Argon2id. Remaining audit findings: SP3-SP10 (4 in `05-share-link-security.md`, 1 in `03-encryption.md`, 3 in data-handling/gdpr). SI-029 still blocked on legal counsel. 17/17 linters green. Score 100/100.
