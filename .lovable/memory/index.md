...
## Memories
- [Spec issue tracker](mem://features/spec-issue-tracker.md) — **1 SI open + 4 audit findings open / 44 SIs closed.** Session 92 (2026-04-29): **drained HU1 (F-class)** — `02-undo-redo.md §48` ULID→UUIDv7. **Repo-wide sweep surfaced 2 missed violations** in `18-analytics-telemetry/01-opt-in-analytics.md §3` (event payload `account_id`/`org_id` typed as `ulid`); also corrected. Repo `rg ulid` now clean. Scorecard restored to 100/100/100. Remaining: HU2+HU3 (S2 — retention 90d vs time-travel 30d drift, `Pro+` not pinned to entitlement SoT), HU4+HU5 (S3 polish). SI-029 still blocked on legal counsel.
