...
## Memories
- [Spec issue tracker](mem://features/spec-issue-tracker.md) — **1 SI open + 4 audit findings open / 32 SIs closed.** Session 84 (2026-04-29): **drained SC1+SC2 (S1 batch)** of sharing-collab audit (`audit-2026-04-29-sharing-collab-sweep-83.md`, now 3/7 closed; SC5 retroactively closed via Session 83 memory path correction). Password-unlock rate-limit prose unified to `13-rate-limit-values.md §4` SoT; public-share blanket `60/min` replaced with explicit two-tier cross-reference. Remaining: SC3+SC4 (S2 — reserved-slug clarification + `/purge` audit/idempotency cross-refs), SC6+SC7 (S3 polish — emoji enum + Markdown-lite subset). SI-029 still blocked on legal counsel.
