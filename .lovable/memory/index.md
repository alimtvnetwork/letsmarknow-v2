...
## Memories
- [Spec issue tracker](mem://features/spec-issue-tracker.md) — Live open/closed counts. Currently 0 open / 31 closed. Score 100/100. Endpoint inventory: 171 canonical declarations. Linters shipped: **14/19 — `error-code-casing` added Session 31** (cross-file: catalog auto-extracted from `03-api-endpoints/18-error-codes.md §3` = 84 codes; flags any backticked SCREAMING_SNAKE token in error-context anywhere in spec that's not in catalog). All green. Real drift catch rate: 4/14 (29%; preventive). Allowlists: 9 files (now supporting per-occurrence `<file>:<TOKEN>` silencing). Tuning lesson S31: per-occurrence allowlists beat rule-weakening when forbidden tokens have legitimate rule-teaching uses (consistent with S28/S29/S30 lessons). Last activity: Session 31, 2026-04-29.
