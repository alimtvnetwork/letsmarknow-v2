...
## Memories
- [Spec issue tracker](mem://features/spec-issue-tracker.md) — **1 SI open + 2 audit findings open / 44 SIs closed.** Session 97 (2026-04-29): **drained EX2 (S3)** — surface-vs-API vocabulary reconciled. `08-sharing-collab/13-share-link.md §1.4` lead-in + table row rewritten to say "address-bar / `webNavigation` URL-pattern", not "omnibox keyword". `04-extension/06-omnibox.md §102` clarifies `lmn` is the only registered `chrome.omnibox` keyword; `lmk` is a `webNavigation` path-pattern intercept. Audit-95 now 2/4 closed. Remaining: EX3+EX4 (S3 polish batch). SI-029 still blocked on legal counsel. Scorecard 100/100/100.
