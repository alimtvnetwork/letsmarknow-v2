...
## Memories
- [Spec issue tracker](mem://features/spec-issue-tracker.md) — **1 SI open + 5 audit findings open / 32 SIs closed.** Session 72 (2026-04-29): **closed S2 batch AU2-AU5** from auth-accounts audit. AU2: `06-sessions.md §11` now references `19-security-privacy/03-encryption.md §3` as JWT-rotation numeric SoT (90d/1d). AU3: `06-sessions.md §1` mandates verifiers reject `roles: "system"` JWTs (`security.jwt_system_role_rejected`). AU4: magic-link callback renamed `/v1/auth/magic/callback` → `/v1/auth/magic-link/callback`; both endpoints now share `magic-link` stem. AU5: OAuth callback locked to `/v1/auth/oauth/:provider/callback` per `03-api-endpoints/03-auth.md`; `02-signup-and-signin.md §1` + `13-rate-limit-values.md §2` aligned. Remaining: AU6-AU10 (S3 polish, single session). SI-029 blocked on legal counsel. Score 100/100.
