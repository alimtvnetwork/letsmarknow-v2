...
## Memories
- [Spec issue tracker](mem://features/spec-issue-tracker.md) — **2 SIs open / 44 SIs closed.** Session 147 (2026-05-03): `03-api-endpoints/` broad sweep (audit-147) opened **SI-030 (S1)** — 162 ULID-shaped `"01J..."` placeholders across 21 files violate Core "UUIDv7 everywhere, never ULID" rule. Phase plan: lint rule → bulk replace to UUIDv7 stub → verify. **SI-029** (privacy-pack legal copy) still blocked on counsel. Recent broad sweeps clean: `15-visualization/` (143), `17-admin-org/` (144), `01-information-architecture/` (145), `06-ui-ux/` (146 — 11 string drifts patched). **Next priorities:** SI-030 fix phases A/B/C; then `12-history-undo/`, `16-notifications-updates/`, `22-infrastructure/` broad sweeps.
