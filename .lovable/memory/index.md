...
## Memories
- [Spec issue tracker](mem://features/spec-issue-tracker.md) — Live open/closed counts. Currently 0 open / 29 closed. Score 100/100. Endpoint inventory: 171 canonical declarations (rebased Session 18 via `endpoint-counts --write`). Linters shipped: 4/19 — all green. **Meta-rule trilogy COMPLETE Session 21**: `endpoint-counts` (Counter Discipline), `audit-cadence` (Audit Cadence), `allowlist-discipline` (Allowlist Discipline) + `naming-convention` (first content-class). Allowlists: 1 (`naming-convention.allowlist.txt`, 22 entries). Last activity: Session 21, 2026-04-29.
