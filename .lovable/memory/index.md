...
## Memories
- [Spec issue tracker](mem://features/spec-issue-tracker.md) — Live open/closed counts. Currently 0 open / 31 closed. Score 100/100. Endpoint inventory: 171 canonical declarations. Linters shipped: **16/19 — `storage-path` added Session 33** (cross-file: catalog = 10 buckets from `22-infrastructure/12-storage-layout.md §1`; rules: unknown-bucket [wide-net + per-occurrence allowlist] and legacy-prefix [unconditional, blocks `lmn-<bucket>` form]; scoped to `22-infrastructure/**` + `11-import-export/**`; 28 files scanned). All green. Real drift catch rate: 4/16 (25%; preventive). Allowlists: 11 files. Remaining: 4 src-dependent linters (`brand-pink-anchor`, `color-label-tokens`, `collection-kind-discriminator`, `toast-placement`), all blocked on lifting no-implementation-mode. **All shippable spec-only linters complete.** Last activity: Session 33, 2026-04-29.
