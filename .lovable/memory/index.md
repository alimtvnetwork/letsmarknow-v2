...
## Memories
- [Spec issue tracker](mem://features/spec-issue-tracker.md) — **1 SI open + 0 audit findings open / 36 SIs closed.** Session 86 (2026-04-29): **closed sharing-collab audit (7/7)** — SC6 `Reaction.emoji` SoT pointer to §8 + Free-plan restriction; SC7 new §2.1 "Markdown-lite subset (locked)" with supported/NOT-supported lists, sanitization allow-list, and 4 KB measured on raw source. SI-029 still blocked on legal counsel. Next suggestions: gap-sweep on `19-security-privacy/` or `12-history-undo/`, build-readiness review of `07-features/17-next-queue.md`, or cross-cutting linter sweep.
