...
## Memories
- [Spec issue tracker](mem://features/spec-issue-tracker.md) — Live open/closed counts. Currently 0 open / 31 closed. Score 100/100. Endpoint inventory: 171 canonical declarations. Linters shipped: **11/19 — `pricing-source` (W-3 lock) added Session 28**. All green. W-issue trio + W-4 channels + link-check (W-5) + pricing-source (W-3) all done. Real drift catch rate: 4/11 (36%). Allowlists: 6 files, all validated by `allowlist-discipline`. Tuning lesson S28: linter scope must match the lock's true intent — bare `$N` is too broad; only plan-cadence-suffixed prices belong under W-3. Last activity: Session 28, 2026-04-29.
