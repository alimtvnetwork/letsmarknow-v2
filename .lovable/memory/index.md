...
## Memories
- [Spec issue tracker](mem://features/spec-issue-tracker.md) — **1 SI open + 10 audit findings open / 32 SIs closed.** Session 66 (2026-04-29): **opened security-privacy audit (10 findings: 2×S1, 6×S2, 2×S3)** — `audit-2026-04-29-security-privacy-sweep-66.md`. **S1 launch-blockers:** SP1 (`03-encryption.md` says bcrypt for user passwords, contradicts Argon2id lock in `09-auth-accounts/03-passwords-and-mfa.md`); SP2 (same for share-link passwords). S2 cluster: `05-share-link-security.md` drift vs locked share-link + embed-widget specs (phantom `/s/{token}` URL, "custom slug" naming, wrong sandbox attrs, non-standard `XFO: ALLOWALL`). SI-029 still blocked on legal counsel. 17/17 linters green. Score 100/100 (audit findings not yet promoted to SIs).
