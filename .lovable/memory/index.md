...
## Memories
- [Spec issue tracker](mem://features/spec-issue-tracker.md) — **1 SI open + 4 audit findings open / 44 SIs closed.** Session 99 (2026-04-29): **opened `audit-2026-04-29-data-model-sweep-99.md`** — first-ever sweep of `02-data-model/`. 4 findings: DM1 (S2 — RLS-section coverage gap; only 1 of 12 entity files has `## RLS`), DM2 (S2 — `08-member.md` doesn't declare itself as user_roles SoT or cite `has_role()` SECURITY DEFINER pattern), DM3 (S3 — zero `on delete` SQL clauses across all entity files; carries S3 from 2026-04-19 audit), DM4 (S3 — per-entity Permissions-matrix cross-ref drift; carries S19). Baseline strong: UUIDv7 clean, role enum locked, color_label locked, soft-delete uniform. DM1+DM2 dock Cursor pass to 99 until drained. SI-029 still blocked on legal counsel. Scorecard 100/99/100.
