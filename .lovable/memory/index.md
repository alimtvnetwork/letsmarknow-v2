...
## Memories
- [Spec issue tracker](mem://features/spec-issue-tracker.md) — **1 SI open + 3 audit findings open / 44 SIs closed + 1 audit finding closed.** Session 100 (2026-04-30): **closed DM2** in `audit-2026-04-29-data-model-sweep-99.md` (1 of 4) — appended `## Role-enforcement contract` to `02-data-model/08-member.md` pinning user_roles SoT + `has_role()` SECURITY DEFINER pattern citation to `19-security-privacy/01-threat-model.md`. Audit-99 still has DM1 (S2 — RLS-section coverage gap, 11 entity files), DM3 (S3 — FK on-delete clauses), DM4 (S3 — per-entity Permissions-matrix cross-ref). DM1 still docks Cursor pass to 99 until drained. SI-029 blocked on legal counsel. Scorecard 100/99/100.
