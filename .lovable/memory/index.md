...
## Memories
- [Spec issue tracker](mem://features/spec-issue-tracker.md) — Live open/closed counts. Currently 0 open / 31 closed. Score 100/100. Endpoint inventory: 171 canonical declarations. Linters shipped: **16/19**, all 16 fully green. Session 36 closed last known dangling cross-ref by adding `### 2.7 Next` sub-section to `06-ui-ux/08-keyboard-input.md` (9 shortcuts with scope column + conflict notes) and fixing 2 wrong refs in `17-next-queue.md` that pointed to non-existent `02-keyboard-shortcuts.md` (slot 02 is `02-theming.md`; correct file is `08-keyboard-input.md`). Remaining: 4 src-dependent linters (blocked on lifting no-implementation-mode), Toby spec re-paste (6 ergonomics defaults), comprehensive re-audit. Last activity: Session 36, 2026-04-29.
