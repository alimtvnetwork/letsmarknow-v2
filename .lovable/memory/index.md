...
## Memories
- [Spec issue tracker](mem://features/spec-issue-tracker.md) — **1 SI open + 0 audit findings open / 44 SIs closed + 16 audit findings closed.** Session 105 (2026-04-30): opened + fully drained `audit-2026-04-30-linter-sweep-105.md` (7 findings, LS1-LS4 S2, LS5-LS7 S3). Normalized 14 audit headers (`audit-type: gap-sweep` → `ad-hoc`; freeform status → bare `closed` + `closed-because`); resolved **17/17 folder collision** by renaming `17-i18n-a11y/` → `24-i18n-a11y/` (admin-org keeps 17, i18n-a11y now at slot 24); restructured i18n overview to canonical 5-section layout; bulk-rewrote 8 spec files' path refs; extended 4 lint allowlists with PR-discipline-compliant entries (review-by 2026-10-27). **All 17 lint sub-checks now green.** Only SI-029 (privacy-pack legal copy, blocked on legal counsel) remains open. Scorecard 100/100/100. **Next priorities (from memory):** gap-sweep `09-auth-accounts/`, gap-sweep `03-api-endpoints/`, gap-sweep `04-extension/`.
