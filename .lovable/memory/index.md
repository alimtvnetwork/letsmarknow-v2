...
## Memories
- [Spec issue tracker](mem://features/spec-issue-tracker.md) — **1 SI open + 8 audit findings open / 32 SIs closed.** Session 78 (2026-04-29): **opened new gap-sweep audit on `11-import-export/`** (`audit-2026-04-29-import-export-sweep-78.md`, 0/8 closed). Findings: IE1+IE2 (S1, status enum drift — `cancelled` UK + 3 conflicting `import_state` enums vs canonical `awaiting_upload|queued|running|succeeded|partial|failed|canceled`), IE3+IE4 (S2, rate-limit SoT split — email-in plan quotas in `08-email-in.md` vs gateway limits in `13-rate-limit-values.md`; API/webhook tier limits in `07-webhooks-and-api-imports.md` not in SoT), IE5 (S2, `dedup_mode` telemetry field with no enum), IE6 (S2, Idempotency-Key contract specified 3 different ways), IE7+IE8 (S3, GDPR bundle filename casing + `:webhook_token`/`{account_token}`/`migration-token` path-param style drift). Drain plan: 3 sessions. Previous audit (LB) fully closed. SI-029 still blocked on legal counsel.
