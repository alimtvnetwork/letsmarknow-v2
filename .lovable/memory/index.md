...
## Memories
- [Spec issue tracker](mem://features/spec-issue-tracker.md) — **1 SI open + 0 audit findings open / 43 SIs closed.** Session 90 (2026-04-29): **closed security-privacy audit (7/7)** — SP6 Sessions retention row pinned to `06-sessions.md §1.1+§1.2` SoTs; SP7 `has_role` signature + `<user-roles>` directive citation + role-enum SoT cross-ref in `01-threat-model.md §29 + §103`. SI-029 still blocked on legal counsel. Next suggestions: gap-sweep on `12-history-undo/` (smaller, fast), `04-extension/` (omnibox `lmk` resolver never cross-audited), or `02-data-model/`; build-readiness review of `07-features/17-next-queue.md`; cross-cutting linter sweep.
