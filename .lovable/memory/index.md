...
## Memories
- [Spec issue tracker](mem://features/spec-issue-tracker.md) — Live open/closed counts. Currently 1 open (SI-026 forward-ref backlog: **13 remaining**, was 21 at S38) / 31 closed. Score 100/100. Endpoint inventory: 171 canonical declarations. Linters shipped: **17/19**, all 17 fully green (1915 backticked refs across 302 files). Drain progression: S39 closed 6 (phase-roadmap requalifications + release-ops stubs at `04-extension/17/18/19`), S41 closed 2 more (`19-security-privacy/06-extension-privacy.md` + `19-security-privacy/07-privacy-policy.md`); folder overviews updated; `13-spec-issues/02-current-issues.md` SI-026 row rewritten to use plain-quoted planned filenames to avoid linter-recursion drift. Remaining: 4 src-dependent linters (blocked on lifting no-implementation-mode), Toby spec re-paste (6 ergonomics defaults), SI-026 closure (13 planned files to author). Session 42 folded Toby "To/Links" ideas into the existing Share model: added `memorable_slug` field to `02-data-model/07-share.md`, expanded `08-sharing-collab/13-share-link.md` with §1.2 (lmk surface), §1.4 (resolver matrix), §7 (orphaned-target repointing), §8 (request-access page); added `lmk/{slug}` shortlink interception §11 to `04-extension/06-omnibox.md`; refreshed glossary. Session 43 confirmed/extended permissions matrix for the new lmk surface: added 3 rows to `08-sharing-collab/05-permissions-matrix.md` §5 (set memorable slug, repoint orphaned target, handle access requests) plus 2 new actions to `permissions-matrix.json` (`share.repoint_target`, `share.handle_access_request`). Note locked: `share.update` already covers memorable-slug changes — no separate permission. Last activity: Session 43, 2026-04-29.
