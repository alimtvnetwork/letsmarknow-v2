...
## Memories
- [Spec issue tracker](mem://features/spec-issue-tracker.md) — **1 open / 32 closed.** SI-029 (S2, privacy-pack stub expansion, blocked on legal counsel). Session 58 (2026-04-29): drained D1+D4 from `audit-2026-04-29-data-model-sweep.md`. D1 (S1 codegen-blocker) closed — `09-history-event.md target_type` now includes account/next_item/session/mfa_factor. D4 (S2) closed — overview §2 table now lists `12-next-item.md`. Audit now 2/9 closed; 7 remaining (D2 overview Share-row drift, D3, D5 search_tsv propagation, D6, D7, D8, D9) all agent-resolvable. 17/17 linters green. Score 100/100.
