...
## Memories
- [Spec issue tracker](mem://features/spec-issue-tracker.md) — Live open/closed counts. Currently 0 open / 30 closed. Score 100/100. Endpoint inventory: 171 canonical declarations. Linters shipped: **10/19 — halfway mark crossed Session 27**. All green. W-issue trio + W-4 channels + link-check (W-5) all done. Real drift catch rate: 3/10 (30%). Allowlists: 5 files, all validated by `allowlist-discipline`. Corpus finding S27: cross-refs use backticked-path style (`` `06-ui-ux/01.md §2` ``) over markdown links — only 34 relative links exist across 294 files; future `backticked-path-validity` linter parked. Last activity: Session 27, 2026-04-29.
