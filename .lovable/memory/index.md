...
## Memories
- [Spec issue tracker](mem://features/spec-issue-tracker.md) — Live open/closed counts. Currently 0 open / 29 closed. Score 100/100. Endpoint inventory: 171 canonical declarations. Linters shipped: 5/19 — all green (`endpoint-counts`, `audit-cadence`, `naming-convention`, `allowlist-discipline`, `folder-overview`). Meta-rule trilogy COMPLETE Session 21. Real drift caught: 12 phantom endpoint-count rows (S18) + 7 heading normalizations across 2 overview files (S22). Allowlists: 2 files, both validated by `allowlist-discipline`. Last activity: Session 22, 2026-04-29.
