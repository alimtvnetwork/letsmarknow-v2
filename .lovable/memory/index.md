...
## Memories
- [Spec issue tracker](mem://features/spec-issue-tracker.md) — **1 open / 32 closed.** SI-029 (S2, blocked on legal counsel). Session 63 (2026-04-29): **closed S1 launch-blocker SH1** + SH8 from sharing-collab audit (5 of 9 closed). `05-permissions-matrix.md` now lists all 7 locked roles; `Guest` column added to all 4 action tables; new §10.5 documents `system` SECURITY DEFINER bypass (no per-action grants needed). `permissions-matrix.json` already had `guest`+`system` in `roles{}`; added codegen `notes[]` for SYSTEM BYPASS + GUEST default-deny. Remaining: SH3, SH5, SH6, SH9. 17/17 linters green. Score 100/100.
