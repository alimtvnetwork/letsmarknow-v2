...
## Memories
- [Spec issue tracker](mem://features/spec-issue-tracker.md) — **1 SI open + 10 audit findings open / 32 SIs closed.** Session 70 (2026-04-29): **opened auth-accounts audit (10 findings: 1×S1, 4×S2, 5×S3)** — `audit-2026-04-29-auth-accounts-sweep-70.md`. **S1 AU1:** refresh-cookie `SameSite=Strict` (in `06-sessions.md`/`10-device-and-security.md`) contradicts `SameSite=Lax` in `19-security-privacy/01-threat-model.md` — Strict would break magic-link sign-in. S2 cluster: JWT rotation language drift, no `system`-claim rejection rule, magic-link path family inconsistent (`/magic-link/send` vs `/magic/callback`), OAuth callback path ambiguity. SI-029 still blocked on legal counsel. Score 100/100 (audit findings not yet promoted to SIs).
