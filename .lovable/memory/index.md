...
## Memories
- [Spec issue tracker](mem://features/spec-issue-tracker.md) — **0 open / 32 closed** in tracker. Open audit: `23-audits/audit-2026-04-29-security-privacy-sweep.md` — **6 of 8 closed** (F1+F2+F3+F8 in S54; F6 Bcrypt→Argon2id and F7 `|`→`→` table arrows in S55, both in `01-threat-model.md`). **2 findings remain, both blocked on user input**: F4 (stub-file launch ownership = needs SI-029 + legal-counsel naming) and F5 (residency-region count: 4 in data-handling vs 2 in policy = product decision). 17/17 linters green. Score 100/100.
