...
## Memories
- [Spec issue tracker](mem://features/spec-issue-tracker.md) — **1 SI open + 0 audit findings open / 44 SIs closed + 9 audit findings closed.** Session 104 (2026-04-30): opened + fully drained `audit-2026-04-30-next-queue-readiness.md` (NQ1 `account_setting` table undeclared S2, NQ2 realtime channel `account:{account_id}:next` missing from topology S2, NQ3 Phase-1 roadmap missing Next bullet S3). All 3 closed same session — added `account_setting` sub-entity to `11-account.md` (8 keys, defaults, RLS), added `account:{account_id}:next` row to `14-realtime-transport.md §2`, added Next bullet to `20-roadmap/02-phase-1-v1.md` Save flows. `07-features/17-next-queue.md` now build-ready. Only SI-029 (privacy-pack legal copy, blocked on legal counsel) remains open. Scorecard 100/100/100. **Next priorities (from memory):** cross-cutting linter sweep (verify CI sub-checks after recent data-model + realtime changes), gap-sweep `09-auth-accounts/`, gap-sweep `03-api-endpoints/`.
