...
## Memories
- [Spec issue tracker](mem://features/spec-issue-tracker.md) — **1 open / 32 closed.** SI-029 (S2, blocked on legal counsel — only remaining open issue). Session 65 (2026-04-29): **sharing-collab audit fully closed (9/9).** SH6: added `Status` column to `00-overview.md §2` table. SH9: added sandbox security note in `10-embed-widget.md` §3 explaining omitted `allow-same-origin`/`allow-top-navigation` and acceptable `allow-popups-to-escape-sandbox`. 17/17 linters green. Score 100/100.
