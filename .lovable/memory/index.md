...
## Memories
- [Spec issue tracker](mem://features/spec-issue-tracker.md) — **1 SI open + 3 audit findings open / 32 SIs closed.** Session 76 (2026-04-29): **closed S2 batch LB3-LB7** from licensing-billing audit. LB3: `max_tier` ranking now `free<pro<team` using License.plan enum directly; Team Enterprise = `plan=team` + `org_entitlement_overrides`. LB4: false alarm — `team_enterprise_yearly` IS in Paddle SKU table line 46. LB5: added Paddle `subscription.trial_will_end` event for `BILL_TRIAL_ENDING` parity. LB6: documented that Paddle conveys PM state via `subscription.updated`+`transaction.completed` (no standalone PM events). LB7: `org_subscription.processor` enum widened to `stripe|paddle` with SoT in `03-stripe-integration.md §5`. Remaining: LB8-LB10 (S3 polish, single session). SI-029 still blocked on legal counsel. Score 100/100.
