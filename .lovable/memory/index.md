...
## Memories
- [Spec issue tracker](mem://features/spec-issue-tracker.md) — **1 SI open + 10 audit findings open / 32 SIs closed.** Session 74 (2026-04-29): **opened licensing-billing audit** (`audit-2026-04-29-licensing-billing-sweep-74.md`, 10 findings: 2×S1, 5×S2, 3×S3). Headlines: LB1 `canceled`/`cancelled` spelling drift breaks Stripe webhook handler + cancellation email; LB2 `amount_*_minor` field names in `08-invoices-and-tax.md` violate locked W-10 rule (`amount_cents` only); LB3 `team_enterprise` in entitlements ranking but absent from `License.plan` enum (clashes with AU8 SoT); LB5/LB6 Paddle missing `trial_will_end` and `payment_method.*` parity. Auth-accounts audit (Session 70) fully drained 10/10. SI-029 still blocked on legal counsel. Score 100/100.
