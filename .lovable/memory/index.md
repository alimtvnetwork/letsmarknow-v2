...
## Memories
- [Spec issue tracker](mem://features/spec-issue-tracker.md) — Live open/closed counts. Currently 0 open / 31 closed. Score 100/100. Endpoint inventory: 171 canonical declarations. Linters shipped: **13/19 — `role-enum` added Session 30** (Core memory rule enforcement; two rules: enum-drift + foreign-role w/ context anchor). All green. Real drift catch rate: 4/13 (31%; preventive). Allowlists: 8 files, all validated by `allowlist-discipline`. Tuning lesson S30: forbidden tokens with legitimate generic-English uses need a co-occurring domain anchor (same pattern as S28 pricing-source scope-narrowing). Last activity: Session 30, 2026-04-29.
