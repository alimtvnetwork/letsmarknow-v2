...
## Memories
- [Spec issue tracker](mem://features/spec-issue-tracker.md) — Live open/closed counts. Currently 0 open / 31 closed. Score 100/100. Endpoint inventory: 171 canonical declarations. Linters shipped: **16/19**. Session 35 closed pre-existing conv-log drift across `link-check` (2 hits, per-occurrence allowlisted), `money-units` (13 + 1 closed-issues hit, file-level), `sku-naming` (5 hits, file-level). **All 16 active linters now fully green.** Remaining: 4 src-dependent linters (`brand-pink-anchor`, `color-label-tokens`, `collection-kind-discriminator`, `toast-placement`), all blocked on lifting no-implementation-mode. Last activity: Session 35, 2026-04-29.
