...
## Memories
- [Spec issue tracker](mem://features/spec-issue-tracker.md) — **1 SI open + 7 audit findings open / 36 SIs closed.** Session 87 (2026-04-29): **opened gap-sweep audit on `19-security-privacy/`** (`audit-2026-04-29-security-privacy-sweep-87.md`, 0/7 closed). Findings: SP1+SP2 (S1, rate-limit SoT drift — mirrors SC1+SC2), SP3 (S2, audit-event naming drift `brute_force_lockout` vs `locked_brute_force` + 3 missing events), SP4 (S2, CAPTCHA thresholds undeclared), SP5 (S2, cookie inventory no SoT), SP6+SP7 (S3, session TTL vague + `has_role` cross-ref). Drain plan: 3 sessions. SI-029 still blocked on legal counsel.
