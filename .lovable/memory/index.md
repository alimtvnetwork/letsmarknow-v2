...
## Memories
- [Spec issue tracker](mem://features/spec-issue-tracker.md) — **1 SI open + 6 audit findings open / 32 SIs closed.** Session 79 (2026-04-29): **drained IE1+IE2 (S1 batch)** of import-export audit (`audit-2026-04-29-import-export-sweep-78.md`, now 2/8 closed). UK `cancelled` → US `canceled` across `03-import-pipeline.md` + `06-large-imports.md`; competing `import_state` enums removed, large-imports checkpoint chain reframed as internal `phase` sub-states under canonical top-level `status=running`. Remaining: IE3+IE4+IE5+IE6 (S2 batch — rate-limit SoT + dedup_mode enum + idempotency contract), IE7+IE8 (S3 polish). SI-029 still blocked on legal counsel.
