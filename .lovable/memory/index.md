...
## Memories
- [Spec issue tracker](mem://features/spec-issue-tracker.md) — Live open/closed counts. Currently 1 open (SI-026 forward-ref backlog) / 31 closed. Score 100/100. Endpoint inventory: 171 canonical declarations. Linters shipped: **17/19**, all 17 fully green. Session 38 shipped `backticked-path-resolution` linter (1843 refs scanned, sister to link-check), fixed 25+ basename drifts in active spec files (e.g. `18-error-codes.md` → `../03-api-endpoints/18-error-codes.md`), allowlisted 21 forward-refs under SI-026, registered in §2.1.1. Built-in skips for frozen files (conv-log, audits, naming-conventions, templates, wireframes). Remaining: 4 src-dependent linters (blocked on lifting no-implementation-mode), Toby spec re-paste (6 ergonomics defaults), SI-026 closure (author planned forward-ref files). Last activity: Session 38, 2026-04-29.
