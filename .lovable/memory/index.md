...
## Memories
- [Spec issue tracker](mem://features/spec-issue-tracker.md) — **1 SI open + 2 audit findings open / 41 SIs closed.** Session 89 (2026-04-29): **drained SP3+SP4+SP5 (S2 batch)** of security-privacy audit (`audit-2026-04-29-security-privacy-sweep-87.md`, now 5/7 closed). SP3: 4 share audit events reconciled (`share.locked_brute_force` rename + 3 added to SoT). SP4: new `13-rate-limit-values.md §2.1` CAPTCHA escalation thresholds + 403 retry contract. SP5: canonical 8-cookie inventory at `06-sessions.md §1.3` (drift-corrected `__Host-session` → `__Host-lmn_refresh`). Remaining: SP6+SP7 (S3 polish — session TTL + `has_role` cross-ref). SI-029 still blocked on legal counsel.
