...
## Memories
- [Spec issue tracker](mem://features/spec-issue-tracker.md) — **1 open / 32 closed.** SI-029 (S2, blocked on legal counsel). Session 64 (2026-04-29): closed SH3 + SH5 from sharing-collab audit (7 of 9 closed). `01-share-model.md` §1 Share table renamed `scope_type/scope_id` → `target_type/target_id` (reconciled with v1 lock); added `memorable_slug` row + v2→v1 reconciliation note clarifying v2 only adds multi-link, embed URLs, custom domain, per-link analytics. Remaining: SH6, SH9 (both S3 doc cleanup). 17/17 linters green. Score 100/100.
