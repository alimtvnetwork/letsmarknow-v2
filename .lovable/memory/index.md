...
## Memories
- [Spec issue tracker](mem://features/spec-issue-tracker.md) — **1 open / 32 closed.** SI-029 (S2, privacy-pack stub expansion, blocked on legal counsel). Session 58 (2026-04-29): drained D1+D2+D4 from `audit-2026-04-29-data-model-sweep.md` (3 of 9 closed). D1 (S1 codegen-blocker): `09-history-event.md target_type` enum extended to include account/next_item/session/mfa_factor. D2: `00-overview.md §2` adds `12-next-item.md` row (audit over-stated; 09 + 11 rows already existed). D4: §2 Share row corrected from `target_kind`/`audience` → `target_type`/`mode`/`memorable_slug`. Remaining: D3, D5 (search_tsv propagation to Space/Collection/Group), D6, D7, D8, D9. 17/17 linters green. Score 100/100.
