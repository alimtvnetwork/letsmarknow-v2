...
## Memories
- [Spec issue tracker](mem://features/spec-issue-tracker.md) — **1 SI open + 5 audit findings open / 38 SIs closed.** Session 88 (2026-04-29): **drained SP1+SP2 (S1 batch)** of security-privacy audit (`audit-2026-04-29-security-privacy-sweep-87.md`, now 2/7 closed). Password-share + public-share rate-limit prose unified to `13-rate-limit-values.md §4` SoT in `01-threat-model.md` + `05-share-link-security.md`. Remaining: SP3+SP4+SP5 (S2 — audit-event reconciliation + CAPTCHA threshold + cookie inventory SoT), SP6+SP7 (S3 polish — session TTL + `has_role` cross-ref). SI-029 still blocked on legal counsel.
