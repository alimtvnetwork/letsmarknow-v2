...
## Memories
- [Spec issue tracker](mem://features/spec-issue-tracker.md) — **1 SI open + 0 audit findings open / 32 SIs closed.** Session 77 (2026-04-29): **closed S3 batch LB8-LB10**, fully closing the licensing-billing audit (10/10). LB8: §7.4 wording fixed to reference "Plan ID listed in `01-plans-matrix.md §6`"; §6 of plans-matrix now defines `plan_code` as the runtime field name carrying Plan ID strings. LB9: Paddle Product IDs rewritten as `<*_paddle_live>` placeholders matching Stripe convention; `_TBD` suffix eliminated. LB10: added `included_seats` column to both SKU tables and codegen target (`5` for `lifetime_team`, null elsewhere). Audit status: **closed**. SI-029 still blocked on legal counsel. Score 100/100.
