...
## Memories
- [Spec issue tracker](mem://features/spec-issue-tracker.md) — Live open/closed counts. Currently 0 open / 31 closed. Score 100/100. Endpoint inventory: 171 canonical declarations. Linters shipped: **15/19 — `env-var-naming` added Session 32** (cross-file: catalog auto-extracted from `22-infrastructure/03-env-vars.md` = 57 vars; flags any `process.env.X` / `import.meta.env.X` / `Deno.env.get('X')` reference whose name isn't in catalog or isn't SCREAMING_SNAKE; scans inside fenced code blocks too — env in code samples counts). All green. Real drift catch rate: 4/15 (27%; preventive). Allowlists: 10 files. Remaining: `storage-path` (W-7, shippable now) + 4 src-dependent linters blocked on lifting no-implementation-mode. Last activity: Session 32, 2026-04-29.
