...
## Memories
- [Spec issue tracker](mem://features/spec-issue-tracker.md) — **1 SI open + 1 audit finding open / 44 SIs closed + 3 audit findings closed.** Session 102 (2026-04-30): **closed DM3** in `audit-2026-04-29-data-model-sweep-99.md` (3 of 4) — added §4a Master FK on-delete table to `02-data-model/00-overview.md` enumerating every cross-entity FK action (cascades, `set null`, `restrict`, polymorphic application-managed, universal Audit-Block rule) + appended `## Foreign keys` pointer block to all 12 entity files. Audit-99 only DM4 (S3 — per-entity Permissions-matrix cross-ref to 10 files) remains. SI-029 blocked on legal counsel. Scorecard 100/100/100.
