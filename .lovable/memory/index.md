...
## Memories
- [Spec issue tracker](mem://features/spec-issue-tracker.md) — **1 SI open + 4 audit findings open / 32 SIs closed.** Session 68 (2026-04-29): closed SP3+SP4+SP5+SP6 from security-privacy audit (6 of 10 closed). `05-share-link-security.md` aligned to locked specs: §1 removed phantom `/s/{token}` URL + added memorable shortlinks; §2 rate-limit updated; §11 embed sandbox now matches `10-embed-widget.md §3`, replaced non-standard `XFO: ALLOWALL` with `CSP: frame-ancestors`. Remaining audit: SP7 (RS256 fallback in `03-encryption.md`), SP8 (sub-processor table triplication), SP9 (EU residency drift), SP10 (TLS 1.2 sunset date). SI-029 still blocked on legal counsel. Score 100/100.
