...
## Memories
- [Spec issue tracker](mem://features/spec-issue-tracker.md) — Live open/closed counts. Currently 0 open / 30 closed. Score 100/100. Endpoint inventory: 171 canonical declarations. Linters shipped: 6/19 — all green (`endpoint-counts`, `audit-cadence`, `naming-convention`, `allowlist-discipline`, `folder-overview`, `money-units`). 3 of 6 caught real drift on first run (50% hit rate): SI-025 (12 phantom endpoint rows), S22 heading drift (3 files), SI-026 (`discount_minor` survivor in W-10 sweep). Allowlists: 3 files, all validated by `allowlist-discipline`. Last activity: SI-026 caught + closed Session 23, 2026-04-29.
