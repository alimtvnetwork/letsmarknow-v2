...
## Memories
- [Spec issue tracker](mem://features/spec-issue-tracker.md) — Live open/closed counts. Currently 1 open (SI-026 forward-ref backlog: **12 remaining**, was 21 at S38) / 31 closed. Score 100/100. Endpoint inventory: 171 canonical declarations. Linters shipped: **17/19**, all 17 fully green (1938 backticked refs across 303 files). Drain progression: S39 closed 6 (phase-roadmap requalifications + release-ops stubs at `04-extension/17/18/19`), S41 closed 2 (`19-security-privacy/06-extension-privacy.md` + `07-privacy-policy.md`), S44 closed 1 (`08-sharing-collab/url-normalization.md` — full pipeline + tracking-strip list + `lmk/` slug normalization). Session 42 folded Toby To/Links into Share model (added `memorable_slug` + lmk surface + resolver matrix + repointing + access-request page). Session 43 extended permissions matrix (`share.repoint_target`, `share.handle_access_request`). Remaining: 4 src-dependent linters (blocked on lifting no-implementation-mode), Toby spec re-paste (6 ergonomics defaults), SI-026 closure (12 planned files to author). Last activity: Session 44, 2026-04-29.
