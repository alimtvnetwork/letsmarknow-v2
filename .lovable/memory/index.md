...
## Memories
- [Spec issue tracker](mem://features/spec-issue-tracker.md) — **1 SI open + 9 audit findings open / 32 SIs closed.** Session 71 (2026-04-29): **closed S1 AU1** from auth-accounts audit. Refresh cookie now `SameSite=Lax` in `06-sessions.md §1` (was Strict — would have broken magic-link); trusted-device cookie kept `SameSite=Strict` in `10-device-and-security.md §2` with explicit "intentional difference" rationale. Aligns with `19-security-privacy/01-threat-model.md §2` Lax baseline. Remaining audit: AU2 (JWT rotation language), AU3 (`system`-claim rejection), AU4 (magic-link path family), AU5 (OAuth callback ambiguity), AU6-AU10 (S3 polish). SI-029 still blocked on legal counsel. Score 100/100.
