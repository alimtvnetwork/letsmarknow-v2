...
## Memories
- [Spec issue tracker](mem://features/spec-issue-tracker.md) — **1 SI open + 8 audit findings open / 32 SIs closed.** Session 75 (2026-04-29): **closed S1 pair LB1 + LB2** from licensing-billing audit. LB1: `cancelled` → `canceled` (US, matches Stripe + Paddle) in telemetry (`plan.canceled`) + `BILL_CANCELLATION_CONFIRMED` email trigger; `rg cancelled` clean. LB2: `amount_*_minor` → `amount_*_cents` in `08-invoices-and-tax.md` Invoice schema; `rg _minor` clean. Remaining: LB3-LB7 (S2 batch), LB8-LB10 (S3 batch). SI-029 still blocked on legal counsel. Score 100/100.
