...
## Memories
- [Spec issue tracker](mem://features/spec-issue-tracker.md) — **1 SI open + 5 audit findings open / 43 SIs closed.** Session 91 (2026-04-29): **opened gap-sweep audit on `12-history-undo/`** (`audit-2026-04-29-history-undo-sweep-91.md`, 0/5 closed). **HU1 = F-class locked-rule violation** (`02-undo-redo.md §48` says "ULIDs", violates Core "Never ULID" rule — depresses Raw-LLM + Cursor passes ~3 pts each until drained). HU2+HU3 (S2 — retention 90d vs time-travel 30d drift, `Pro+` not pinned to entitlement SoT). HU4+HU5 (S3 — Idempotency-Key cross-ref + history endpoint SoT pointer). Drain plan: 3 sessions; HU1 first. SI-029 still blocked on legal counsel.
