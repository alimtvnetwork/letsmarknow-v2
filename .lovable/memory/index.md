...
## Memories
- [Spec issue tracker](mem://features/spec-issue-tracker.md) — **1 SI open + 0 audit findings open / 32 SIs closed.** Session 73 (2026-04-29): **closed S3 batch AU6-AU10** — auth-accounts audit now fully drained (10/10). AU6: `password_hash` annotation inlines argon2 params. AU7: `Org.kind`/`plan_id` allowed combinations explicit. AU8: `Org.plan_id` declared a denormalization of `License.plan` (SoT: `02-data-model/10-license.md §1`); lifecycle states live on `License.status`. AU9: closed incidentally in S72. AU10: HIBP live-check trigger lowered to ≥ 8 chars + `on blur`. Only SI-029 remains (blocked on legal counsel). Score 100/100.
