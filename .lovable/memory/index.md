...
## Memories
- [Spec issue tracker](mem://features/spec-issue-tracker.md) — **1 SI open + 2 audit findings open / 44 SIs closed.** Session 93 (2026-04-29): **drained HU2 + HU3 (S2 batch)** — added entitlement keys `features.history.retention_days` + `features.history.time_travel` to `10-licensing-billing/01-plans-matrix.md §8`; reconciled `02-undo-redo.md §2/§4/§13`, `01-event-log.md §7`, and `12-history-undo/readme.md §26` to reference per-plan retention (Free 7d / Pro 90d / Team 1y / Ent 7y) and cite the entitlement SoT instead of the prior "30 days … (Pro+)" drift. Audit `audit-2026-04-29-history-undo-sweep-91.md` now 3/5 closed. Remaining: HU4+HU5 (S3 polish — Idempotency cross-ref + history endpoint SoT pointer). SI-029 still blocked on legal counsel. Scorecard 100/100/100.
