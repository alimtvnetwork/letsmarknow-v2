...
## Memories
- [Spec issue tracker](mem://features/spec-issue-tracker.md) — **1 open / 32 closed.** SI-029 (S2, privacy-pack stub expansion, blocked on legal counsel). Session 57 (2026-04-29): authored open audit `23-audits/audit-2026-04-29-data-model-sweep.md` — 9 findings (1×S1, 5×S2, 3×S3). Headline D1 (S1 codegen-blocker): `09-history-event.md target_type` enum missing `next_item` + `account`. Other findings: D2 overview-table missing 3 of 13 file rows; D4 `target_kind` vs `target_type` + non-existent `audience` field naming drift in `00-overview.md` Share row; D5 `search_tsv` migration partial (only `05-item.md` migrated, Space/Collection/Group still on legacy "Full-text on …"); D3+D6+D7+D8+D9 polish. All 9 are agent-resolvable in 3–4 sessions, no user input needed. 17/17 linters green. Score 100/100.
