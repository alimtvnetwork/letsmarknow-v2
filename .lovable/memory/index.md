...
## Memories
- [Spec issue tracker](mem://features/spec-issue-tracker.md) — **1 SI open + 2 audit findings open / 32 SIs closed.** Session 80 (2026-04-29): **drained IE3+IE4+IE5+IE6 (S2 batch)** of import-export audit (`audit-2026-04-29-import-export-sweep-78.md`, now 6/8 closed). Email-in & API-token rate-limit SoTs cross-referenced; canonical `dedup_mode` telemetry enum (`skip|merge|allow`) declared in `11-dedup-algorithm.md §6`; Idempotency-Key contract unified to `01-conventions.md §6` SoT across 3 files. Remaining: IE7+IE8 (S3 polish — GDPR filename casing + path-param style drift). SI-029 still blocked on legal counsel.
