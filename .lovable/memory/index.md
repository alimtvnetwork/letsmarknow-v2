...
## Memories
- [Spec issue tracker](mem://features/spec-issue-tracker.md) — **1 SI open + 4 audit findings open / 44 SIs closed.** Session 95 (2026-04-29): **opened `audit-2026-04-29-extension-sweep-95.md`** — first-ever sweep of `04-extension/`. 4 findings: EX1 (S2 — manifest missing `webNavigation` permission required by `lmk/{slug}` interceptor in `06-omnibox.md §11`), EX2 (S3 — share-link 13-share-link.md §1.4 mislabels `lmk` resolver as omnibox keyword; it is `webNavigation` URL-pattern), EX3 (S3 — `19-staging-seed.md` role-pattern SoT cross-ref polish), EX4 (S3 — `11-auth-bridge.md §4` Idempotency-Key SoT cross-ref polish). EX1 docks Cursor pass to 99 until drained. SI-029 still blocked on legal counsel. Scorecard 100/99/100.
