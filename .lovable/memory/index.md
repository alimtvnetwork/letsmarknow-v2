...
## Memories
- [Spec issue tracker](mem://features/spec-issue-tracker.md) — **1 SI open + 2 audit findings open / 34 SIs closed.** Session 85 (2026-04-29): **drained SC3+SC4 (S2 batch)** of sharing-collab audit (`audit-2026-04-29-sharing-collab-sweep-83.md`, now 5/7 closed). Reserved-slug list scoped + `org_handle` namespace clarified; `/v1/shares/:id/purge` contract fully written (Idempotency-Key, `share.purged` audit event, 90-d analytics retention termination). Remaining: SC6+SC7 (S3 polish — `Reaction.emoji` enum + `Comment.body` Markdown-lite subset). SI-029 still blocked on legal counsel.
