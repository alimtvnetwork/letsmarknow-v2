...
## Memories
- [Spec issue tracker](mem://features/spec-issue-tracker.md) — **1 SI open + 2 audit findings open / 44 SIs closed + 2 audit findings closed.** Session 101 (2026-04-30): **closed DM1** in `audit-2026-04-29-data-model-sweep-99.md` (2 of 4) — created `02-data-model/templates/entity-rls.md` template + appended `## RLS` sections to all 11 entity files (Organization, Space, Collection, Group, Item, Tag, Share, Member, HistoryEvent, License, Account). Cursor pass restored to 100. Audit-99 still has DM3 (S3 — FK on-delete clauses) + DM4 (S3 — per-entity Permissions-matrix cross-ref). SI-029 blocked on legal counsel. Scorecard 100/100/100.
