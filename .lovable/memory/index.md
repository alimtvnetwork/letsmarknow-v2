...
## Memories
- [Spec issue tracker](mem://features/spec-issue-tracker.md) — **0 open / 32 closed** in tracker. Open audit: `23-audits/audit-2026-04-29-security-privacy-sweep.md` — 8 findings, **4 closed S54** (F1 sub-processor list unification → all three files mirror `07-privacy-policy.md §3`; F2 Resend-primary/Postmark-failover propagated; F3 AWS/Cloudflare leakage replaced with Lovable Cloud; F8 orphan "Plain/front" dropped). Bonus: dropped Self-hosted Sentry/PostHog row, renamed `message_id (Postmark)` → `provider_message_id` in `16-billing-emails.md` for provider-agnostic schema. **4 findings remain** (F4 stub-file launch ownership = needs SI-029 + legal-counsel naming; F5 residency-region count mismatch = product decision; F6 Bcrypt→Argon2id typo in `01-threat-model.md` line 36; F7 `|` vs `→` table-arrow typo lines 109–110). 17/17 linters green. Score 100/100.
