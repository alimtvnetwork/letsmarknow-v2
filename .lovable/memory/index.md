...
## Memories
- [Spec issue tracker](mem://features/spec-issue-tracker.md) — **1 SI open + 3 audit findings open / 44 SIs closed.** Session 96 (2026-04-29): **drained EX1 (S2)** — added `webNavigation` to `01-manifest.md` permissions array + Permission Rationale row citing `06-omnibox.md §11` + `08-sharing-collab/13-share-link.md §1.4`. `lmk/{slug}` interceptor surface now manifestly enabled. Audit-95 now 1/4 closed. Cursor pass restored to 100. Remaining: EX2 (S3 — surface-vs-API confusion across share-link + omnibox files), EX3+EX4 (S3 polish batch). SI-029 still blocked on legal counsel. Scorecard 100/100/100.
