...
> A future v2 may allow deeper nesting. Schema must reserve room (see `02-data-model/04-group.md` — `parent_group_id` field allowed but constrained to NULL in v1).
...
| Organization | ❌ | shared via Member invites only (`08-sharing-collab/04-invite-only-shares.md`) |
| Space | ✅ | yes |
| Collection | ✅ | yes |
...
- Soft-deleted entities are restorable via Undo or via the Trash UI (see `12-history-undo/` and `12-history-undo/02-undo-redo.md`).
...
- **GDPR / DSR override:** a verified Data Subject Request (right-to-erasure) bypasses the 30-day grace and triggers immediate hard-delete of the requesting Account's PII and any Org where they are sole Owner. See `19-security-privacy/04-gdpr-ccpa.md` and `17-admin-org/05-data-export-delete.md` for the precedence chain.

### 3.6 Counting toward plan limits

Per Organization (Free tier limits — exact numbers in `10-licensing-billing/01-plans-matrix.md`):
...
- Public Share slugs are separate from `id` (see `08-sharing-collab/01-share-model.md`).
...
| Max members per Organization | 500 (Team) / 5 (Pro) / 1 (Free) | per `10-licensing-billing/01-plans-matrix.md` |
...
| Two users edit same Item simultaneously | Last-write-wins on per-field basis with optimistic concurrency token (`updated_at`). Conflict resolution: see `12-history-undo/03-conflict-resolution.md`. |
...
| Account deletion | Transfers ownership of any Org where they are sole Owner is required first (or Org is deleted with grace period). See `19-security-privacy/04-gdpr-ccpa.md`. |
...
- Plan limits in numeric detail → `10-licensing-billing/01-plans-matrix.md`
