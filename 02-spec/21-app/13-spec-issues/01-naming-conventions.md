...
> **Purpose.** The single objective standard every spec audit compares against. If a file in `02-spec/21-app/**` violates a rule here, it becomes an issue in `02-current-issues.md`.
...
- Always relative from `02-spec/21-app/` root: `02-data-model/05-item.md §3.1`.
...
```
grep -rhoE '`(GET|POST|PUT|PATCH|DELETE) /v1/[^` ?&]+`' 02-spec/21-app/ \
  | tr -d '`' | sort -u
# Then subtract:
grep -rhoE '~~WITHDRAWN: (GET|POST|PUT|PATCH|DELETE) /v1/[^~]+~~' 02-spec/21-app/ \
