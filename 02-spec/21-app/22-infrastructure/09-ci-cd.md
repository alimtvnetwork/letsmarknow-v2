...
| `folder-overview` | `scripts/lint/folder-overview.ts` | Every directory under `02-spec/21-app/` (recursively, excluding hidden dirs and the root itself) MUST contain a file named exactly `00-overview.md`. The file MUST be ≥ 40 lines (proxy for "not a stub") AND MUST contain the headings `## 1. Responsibilities`, `## 2. File-by-file behaviour` (or `behavior`), `## 3. Tasks performed by this folder`, `## 4. What this folder is NOT`, `## 5. Cross-references`. Canonical template: `../templates/folder-overview.md`. Allowlist: `scripts/lint/folder-overview.allowlist.txt` (folders explicitly exempt — currently empty; `02-spec/21-app/templates/` is auto-exempt as a non-domain folder). | F-FOLDER-OVERVIEW (retrospective §8.1). |
...
| `next-singleton-invariants` | `scripts/lint/next-singleton-invariants.ts` | Five cross-cutting checks for the Next feature (`07-features/17-next-queue.md`): (a) `Collection.kind` enum must list all three values `manual\|session\|next`. (b) `next_item.source_kind` value drift — locked to 5 values: `hover_toolbar, popup_save, item_menu, keyboard, command_palette` (others fail). (c) UI verb synonyms ban: "Add to To-do", "Save for later", "Bookmark for Next", "Add to Toby Next" all fail — locked verb is "Add to Next". (d) Scope-phrase ban: "per-workspace Next", "per-Org Next", "Next per Space" fail — Next is per-Account by lock. (e) Realtime channel template for Next must be exactly `account:{account_id}:next` (W-4 curly-brace form). | Next feature lock (added 2026-04-29 alongside the feature spec). |
| `backticked-path-resolution` | `scripts/lint/backticked-path-resolution.ts` | Every backticked markdown-path string in spec prose (form: backticks around a slash-segmented path ending in dot-md, optional anchor) MUST resolve to an existing file via either (a) relative to the referencing file's directory, or (b) relative to spec root. Fenced code blocks and markdown-link bodies are skipped (link-check owns the latter). Built-in skips: `../00-conversation-log.md`, `../23-audits/`, `../templates/`, `../06-ui-ux/wireframes/`, `../13-spec-issues/01-naming-conventions.md`, `../13-spec-issues/03-phase-plan.md`, `../13-spec-issues/04-closed-issues.md`, `../15-visualization/readme.md`, and bare-basename `README.md` / `Readme.md` (export-bundle artifacts). | Sister to `link-check`; closes the gap surfaced in Session 36 where `../07-features/17-next-queue.md` §13 referenced a non-existent shortcuts file and no linter caught it (the corpus's primary cross-ref convention is backticked paths in prose, not markdown links). Tracked under SI-026 for forward-ref backlog. |
| `endpoint-counts` | `scripts/lint/endpoint-counts.ts` | (a) Walks every table row across `03-api-endpoints/0[1-9]-*.md` … `03-api-endpoints/1[0-7]-*.md` (excludes `00-overview.md` and `../03-api-endpoints/18-error-codes.md`), parses the leading `METHOD /path` cell, and computes: total rows, distinct `(METHOD, path)` pairs, and per-method counts (`GET, POST, PATCH, PUT, DELETE`). (b) Re-reads `03-api-endpoints/00-overview.md §7` and asserts the printed numbers match the computed ones exactly. Off-by-one fails. (c) Asserts `distinct ≤ total` and that the difference equals the number of duplicate rows the script enumerates in its error message (so authors see *which* paths are duplicated). (d) Asserts every method bucket sum equals `total`. (e) On `--write` flag (used only by a maintainer-run script, never CI), regenerates §7 in place — CI itself runs read-only. | Counter Discipline rule (added 2026-04-29 after the 181→182 off-by-one in `00-overview.md §7`). Closes the audit gap that required manual Python sweeps. |
...
| `audit-cadence` | `scripts/lint/audit-cadence.ts` | Enforces the **Audit Cadence** meta-rule (see §2.1.4). For every file matching `02-spec/21-app/23-audits/audit-*.md`: (a) MUST contain a top-of-file metadata block with `audit-date: YYYY-MM-DD`, `next-audit-by: YYYY-MM-DD`, `audit-type: <one of: ai-readiness, endpoint-sweep, glossary, parity, retrospective, post-fix, ad-hoc>`, and `status: <open|closed|superseded>`. (b) `next-audit-by` MUST be ≤ 365 days after `audit-date`. (c) Any audit with `status: open` whose `next-audit-by` is in the past fails CI — forces a deliberate re-audit or status change. (d) Per `audit-type`, at most one audit may carry `status: open` at a time (newer one closes/supersedes the older); orphan opens fail. (e) `23-audits/00-overview.md` MUST contain a table listing every audit file with its type, date, status, and next-audit-by — auto-generated from the metadata blocks; out-of-sync rows fail. (f) `../23-audits/audit.md` (the legacy un-prefixed file) and `readme.md` are exempt; `audit-*.md` is the only enforced glob. | Audit Cadence meta-rule (added 2026-04-29). Prevents the failure mode where audits accumulate as historical artifacts and silently go stale — the corpus has 22 audit files spanning 11 days; without cadence enforcement, a 100/100 score from April 19 could still be cited a year from now even after major spec drift. Sibling to Counter Discipline + Allowlist Discipline. |
| `ulid-placeholder` | `scripts/lint/ulid-placeholder.ts` | Bans ULID-shaped placeholders (regex `(^|["'\[\(,\s])01[A-HJ-NP-Z]\.{3}` — Crockford base32 prefix `01[A-Z]` followed by `...`) anywhere in `02-spec/21-app/**.md` and `**.mmd`. Canonical UUIDv7 placeholder is `0190a4f1-6c5e-7c2a-9b3f-1234567890ab`. Allowlist (`scripts/lint/ulid-placeholder.allowlist.txt`): meta files that *describe* the violation (conversation log, SI tracker, audit reports). Any new audit citing the old `01J...` form must be added to the allowlist. | SI-030 (added 2026-05-03). Closes the Core "UUIDv7 everywhere, never ULID" rule — previously violated by 179 occurrences across `03-api-endpoints/` (162) and 8 satellite files (17). |
...
**Schema** — every file matching `02-spec/21-app/23-audits/audit-*.md` MUST start with this metadata block (HTML comment block, so it doesn't render in the audit body):
