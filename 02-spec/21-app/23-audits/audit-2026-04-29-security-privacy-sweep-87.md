...
**Scope:** All 8 markdown files (~930 lines) in `02-spec/21-app/19-security-privacy/`, cross-checked against `09-auth-accounts/13-rate-limit-values.md §4` (rate-limit SoT — re-canonized in Sessions 84 + 85), `08-sharing-collab/09-audit-log.md` (share-event SoT), `09-auth-accounts/03-passwords-and-mfa.md §1` (Argon2id SoT), `00-overview/02-glossary.md` (role enum SoT).
