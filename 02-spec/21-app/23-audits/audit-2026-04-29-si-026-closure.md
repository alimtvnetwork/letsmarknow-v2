...
S47 hit a collision: planned slot `07` in `10-licensing-billing/` was occupied, forcing `16-billing-emails.md`. **Mitigation:** before authoring, run `ls 02-spec/21-app/{folder}/` and pick the next free NN.
...
After authoring a new file, 2–3 other files typically still cited the *old aspirational name*. Without renaming those, the linter would resurface a near-identical ref. **Mitigation:** after authoring, run `rg -l "old-name.md" 02-spec/21-app/` and rename in the same session.
