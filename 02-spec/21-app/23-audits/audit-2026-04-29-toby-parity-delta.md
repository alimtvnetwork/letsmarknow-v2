...
|---|---|---|
| `02-spec/21-app/00-conversation-log.md` | **NEW** — verbatim instruction log scaffold + first two entries | +56 |
| `02-spec/21-app/13-spec-issues/02-current-issues.md` | Added SI-021 row + 8-step sub-task list | +14 |
| `02-spec/21-app/00-overview/02-glossary.md` | Added §"External-product mappings" with Toby split table + rationale | +18 |
| `02-spec/21-app/07-features/04-collections.md` | Added §13 "Toby parity" (toolbar, star-pin, drag-drop matrix, undo toast, keyboard, visual style, open behaviour, telemetry, open questions) | +138 |
| `02-spec/21-app/02-data-model/03-collection.md` | Added `starred_pin_position` field + invariant + event | +3 |
| `02-spec/21-app/02-data-model/05-item.md` | Added `starred_pin_position` + `color_label` enum + 2 invariants + 2 events | +5 |
| `02-spec/21-app/04-extension/16-open-tabs-panel.md` | **NEW** — 15-section surface spec (data model, layout, interactions, messaging, permissions, privacy, telemetry, entitlements, error codes) | +180 |
| `02-spec/21-app/06-ui-ux/01-design-tokens.md` | Re-anchored brand ramp to Toby pink `#EC4868` (HSL `343 79% 60%`); added §1.6 `--color-label-*` for both light + dark | +35 |
| `.ai-memory/memory/index.md` | Added 3 Core rules (brand color, Workspace split, color_label enum) + SI-021 reference | +4 |
| `.ai-memory/memory/features/spec-issue-tracker.md` | Updated counts (open 0 → 1) + SI-021 entry | +2 |
