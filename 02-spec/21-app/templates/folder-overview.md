...
Bulleted list of every other spec location that consumes or feeds this folder. Each line is `{topic}: {path/to/file.md} [§section if useful]`. Paths must be relative to `02-spec/21-app/` and MUST resolve (the `spec-drift-linter` `link-check` and `folder-overview` sub-checks enforce this — see `22-infrastructure/09-ci-cd.md` §2.1.1).
...
- **Tone.** Match the rest of `02-spec/21-app/`: declarative, locked-rule voice. No "we will", "could", "might". Use "does", "is", "MUST".
...
- **Linter contract.** Spec PRs that add a new folder under `02-spec/21-app/` MUST also add a `00-overview.md` based on this template, or the `folder-overview` sub-check (`22-infrastructure/09-ci-cd.md` §2.1.1) fails the PR. F-FOLDER-OVERVIEW closure is non-regressable.
