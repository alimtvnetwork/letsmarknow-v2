...
| GET | `/v1/sync/since` | bearer+org | Delta-sync poll: returns entities (items, collections, groups, tags) changed since `?cursor=<opaque>` for offline reconciliation. Returns `410 GONE` with code `GONE_CURSOR_STALE` if the cursor is too old to replay. | `../04-extension/10-sync-and-offline.md` |
| GET | `/v1/sync/full` | bearer+org | Full snapshot of an Org's items + collections + groups + tags for first-install or post-`GONE_CURSOR_STALE` recovery. Returns initial `next_cursor` for subsequent `/v1/sync/since` calls. | `../04-extension/10-sync-and-offline.md` |
...
| POST | `/v1/webhooks/email-bounce` | webhook-sig (Resend/Postmark) | — | Bounce + complaint events from email provider; updates `accounts.email_bounced`, suppression. | `22-infrastructure/11-email-provider.md §4` |
