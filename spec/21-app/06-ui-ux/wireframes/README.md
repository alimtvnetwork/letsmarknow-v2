...
> **Purpose:** ASCII-annotated layout specs for every primary surface. AIs and humans use these as the "shape" reference; `design-tokens.md` provides the visual style; `copy-strings.md` provides the text.
>
...
- **Annotations** in `[brackets]` are component names from `component-library.md`.
- **Copy keys** in `{curly.braces}` reference `copy-strings.md`.
- **Density** assumes "Cozy" mode (`data-density.md`) at 1280×800.
- **Breakpoint notes** are in each file.
- **Empty/error/loading** variants follow `empty-error-loading.md`.
