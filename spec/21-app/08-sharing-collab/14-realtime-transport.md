...
| `space:{space_id}` | One per opened Space | Members currently viewing | `presence.sync`, `item.created`, `item.updated`, `item.deleted` |
| `share:{share_token}` | One per opened public share | Anonymous + named viewers | `presence.sync`, `comment.created`, `reaction.created` |
| `user:{user_id}` | Personal | Only the user across devices | `notification.new`, `quota.warning`, `session.revoked` |

**Channel join policy:** Server-side `realtime.authorization` policies mirror the same RLS policies generated from `permissions-matrix.json`. A user joining `space:xyz` MUST satisfy `space.read` on entity `xyz`.
...
- **Presence payload (per user):**
  ```json
  { "user_id": "uuidv7", "display_name": "Ana", "avatar_url": "...", "cursor": { "view": "list", "item_id": "uuidv7|null" }, "last_seen": "2026-04-19T08:00:00Z" }
  ```
...
| Use case | Mechanism |
|---|---|
| User-typed comment | Broadcast (sub-50 ms) THEN DB insert (idempotent on `client_msg_id`) |
...
- **Tie-break:** Lower `user_id` UUIDv7 wins (deterministic).
- **Compound moves** (drag item between collections): wrapped in DB transaction; if 409 conflict, client retries with new parent revision once, then surfaces toast.
