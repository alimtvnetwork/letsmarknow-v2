...
| SI-011 | S1 | `permissions-matrix.json` has 8 roles but locked enum has 7 | `permissions-matrix.json` exposes roles: `owner, admin, editor, viewer, billing, share_viewer, guest, system`. Locked `org_role` enum in `17-admin-org/03-roles.md §1` and `02-data-model/08-member.md` is **7 values** (no `share_viewer`). | `08-sharing-collab/permissions-matrix.json` (decide: remove `share_viewer` OR document as Share-only pseudo-role outside `org_role`) | §4 (identifier parity) |
