...
**Last updated:** 2026-04-20 (after Phase 13.1+13.2 — SI-018/019 closed via convention lock).
...
| SI-020 | S1 | 38 endpoints referenced spec-wide but missing from `03-api-endpoints/00-overview.md` | After excluding SI-018 path-style and SI-019 aliases (now policy-closed in `01-conventions.md §1.1` and §16), 38 distinct method+path pairs are used in domain files but not present in the overview index. Examples: `POST /v1/billing/checkout/session`, `POST /v1/realtime/ticket`, `POST /v1/flags/evaluate`, `GET /v1/jobs/:job_id`, `POST /v1/me/gdpr-export`, `DELETE /v1/mindmap-layouts/:id`, `POST /v1/webhooks/email-in`, `POST /v1/internal/feedback`. Reproducible via the recipe in §"Discovery method" below. | Add the 38 missing rows to the appropriate `03-api-endpoints/NN-*.md` per-domain file AND surface them in `00-overview.md`. Likely owners: `03-auth.md`, `10-shares.md`, `15-import-export.md`, `17-billing-webhooks.md`, plus new sections for `realtime`, `flags`, `jobs`, `mindmap-layouts`, `internal/feedback`, `webhooks/email-in`. Also normalize `{id}`→`:id` and aliases→canonical per §1.1 + §16 conformance greps in the same sweep. | §1, §3 |
