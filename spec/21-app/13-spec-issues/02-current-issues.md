...
**Last updated:** 2026-04-20 (after Phase 13.3 classification — SI-020 split into SI-020a/b/c/d).
...
| SI-020b | S2 | 14 endpoints are aliases of canonical declared paths (extends SI-019) | Casual references in feature/extension/auth files use shorthand paths whose canonical forms ARE declared in `00-overview.md`. Confirmed mismatches: `POST /v1/auth/forgot` → canonical `POST /v1/auth/password/forgot` (`03-auth.md:192`); `POST /v1/auth/magic/request` → canonical declared path TBD verify; `POST /v1/billing/checkout/session` → canonical `POST /v1/organizations/:id/billing/checkout` (`16-licenses.md:95`); `POST /v1/billing/portal/session` → canonical `POST /v1/organizations/:id/billing/portal`; `POST /v1/items/batch` and `POST /v1/items/bulk` → canonical `POST /v1/items:batch` (which itself was forbidden in §16 — see SI-019 contradiction); `POST /v1/bulk` → canonical bulk-ops endpoint per `08-items.md:173`; `PATCH /v1/collections/:collection_id` → canonical `PATCH /v1/collections/:id`. | (a) Extend §16 alias table in `01-conventions.md` with these 14 mappings, (b) sweep referencing files to use canonical paths, (c) reconcile the `items:batch` vs `items/batch` contradiction (SI-019 forbade colon-form, but `00-overview.md` declares it). | §1 |
...
| SI-020d | S2 | 3 phantom paths (count `[0]`) need source verification | `POST /v1/organizations/:id/deletion`, `POST /v1/organizations/:id/exports`, `POST /v1/organizations/:id/invites` showed up in the Phase-12 grep but a follow-up file-trace returned 0 hits outside `13-spec-issues/`. Likely the original grep matched substrings (e.g. `/v1/organizations/:id/billing/exports` partial-matches `/v1/organizations/:id/exports`). | Re-run a stricter grep (full-line match, not substring) to confirm whether these are real ghosts or genuine references. If genuine, fold into SI-020c. If phantom, close as detection error. | §1 |
