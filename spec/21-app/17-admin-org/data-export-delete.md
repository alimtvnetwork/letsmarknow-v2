...
- Import pipeline detailed in `11-import-export/import-pipeline.md`.
...
- Spec'd in `11-import-export/gdpr-export.md`.
...
- For each Org user is in:
  ...
  - Shared Orgs → user removed; their content reassigned to Owner per Org policy (per `members-management.md` § 7).
- Auth identity purged after grace (email + password hash gone).
- 30-day reversibility; afterwards permanent.
- Spec linked in `09-auth-accounts/account-deletion.md`.
