...
status: in_progress (0 of 4 closed)
...
|---|---|---|---|
| EX1 | **S2** | **Manifest is missing `webNavigation` permission required by the `lmk/{slug}` shortlink interceptor.** `06-omnibox.md §11` declares: "the extension intercepts via `chrome.webNavigation.onBeforeNavigate` and resolves against the active Organization." But `01-manifest.md §96-106` `permissions` array does not list `webNavigation`. Without that permission, `chrome.webNavigation.*` listeners silently no-op (Chrome never delivers events) and the entire `lmk/{org_handle}/{slug}` shortlink address-bar surface — locked in Core memory and `08-sharing-collab/13-share-link.md §1.4` — is broken on day one. Fix: add `"webNavigation"` to the `permissions` array, add a row to the Permission Rationale table ("Resolve `lmk/{slug}` shortcut typed in address bar against the active Organization"), and update `01-manifest.md` Store-listing impact (the install dialog wording changes — Chrome describes `webNavigation` as "Read your browsing history"; this overlaps with the existing `tabs` justification so the user-visible string does not get worse). | `01-manifest.md §96-106 + §137-152` |
| EX2 | **S3** | **Surface-vs-API confusion: `08-sharing-collab/13-share-link.md §1.4` calls the `lmk` resolver an "omnibox keyword", but it is NOT an omnibox-API surface.** `13-share-link.md §1.4` says: "When the Mark Now Chrome extension is installed and the user is signed in, typing `lmk/{slug}` in the omnibox is intercepted (per `04-extension/06-omnibox.md` — keyword `lmk`)." This is misleading on two counts: (a) Chrome's `chrome.omnibox` API requires a registered manifest `keyword` followed by a SPACE; only `lmn ` is registered (`01-manifest.md §54-56`). (b) The `lmk/{slug}` interception is a `chrome.webNavigation.onBeforeNavigate` URL-pattern intercept, NOT a keyword route. `06-omnibox.md §11` correctly distinguishes the two with the comparison table; the share-link file should mirror that vocabulary so that any reader (or future implementer AI) does not look for an `lmk` keyword in the manifest. Fix: rewrite the share-link §1.4 lead-in to say "intercepted via `chrome.webNavigation.onBeforeNavigate` URL-pattern listener (per `04-extension/06-omnibox.md §11`) — not the `chrome.omnibox` keyword API". Also delete the line in `06-omnibox.md §102` that asserts "Both keywords (`lmn`, `lmk`) are reserved at registration time" — only `lmn` is a registered keyword; `lmk` is a path-pattern. | `08-sharing-collab/13-share-link.md §1.4`, `04-extension/06-omnibox.md §11 + §102` |
