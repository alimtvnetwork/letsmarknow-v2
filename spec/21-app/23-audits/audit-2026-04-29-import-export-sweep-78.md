...
status: in_progress (0 of 8 closed)
...
|---|---|---|---|
| IE1 | **S1** | **`cancelled` (UK) drift — 3 instances.** `03-import-pipeline.md` line 112 (`import_state=cancelled`), line 170 (telemetry `import.cancelled`), and `06-large-imports.md` line 105 (telemetry `large_import.cancelled`) all use UK spelling. Canonical enum at `03-api-endpoints/15-import-export.md` line 250 is `canceled` (US, matches Stripe/Paddle). Also `06-large-imports.md` line 79 prose says "Cancelled at …" — capitalized UK. Same root-cause class as LB1 (Session 75). | `03-import-pipeline.md`; `06-large-imports.md` |
| IE2 | **S1** | **`import_state` enum drift from canonical SoT.** `03-import-pipeline.md` uses `import_state=cancelled` and `import_state=resumed_from=<batch_id>` (line 98). `06-large-imports.md` §3 uses a separate state machine: `uploaded → parsed → preview_ready → committing(offset=N) → committed`. Canonical SoT is `03-api-endpoints/15-import-export.md` line 250: `awaiting_upload \| queued \| running \| succeeded \| partial \| failed \| canceled`. Three different enums for one concept. Pick one (the canonical wire enum) and document large-imports' internal stages as a sub-state of `running`, not as a competing top-level enum. | `03-import-pipeline.md`; `06-large-imports.md`; `03-api-endpoints/15-import-export.md` (cross-link from both) |
