...
| LB6 | **S2** | **Paddle webhook lacks `payment_method.*` parity.** Stripe handlers cover `payment_method.attached` + `payment_method.detached` (used for "card on file" UI state). Paddle table has no PM events. Either Paddle exposes this differently (state on subscription object — needs documenting) or coverage gap. | `12-billing-webhooks.md`; `04-paddle-integration.md` |
| LB7 | **S2** | **`org_subscription.processor` enum declared single-valued in Stripe spec.** `03-stripe-integration.md §...` declares `processor` as `enum | stripe`. But `04-paddle-integration.md` writes the same column with `processor='paddle'`. The enum SoT must be in the data-model file (`02-data-model/...`) and include both values, not be redeclared per-provider. | `03-stripe-integration.md §...`; canonical enum SoT in `02-data-model/` |
